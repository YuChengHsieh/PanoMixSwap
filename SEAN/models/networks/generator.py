...
class SPADEGenerator(BaseNetwork):
    ...
    def forward(self, input, rgb_img, obj_dic=None, style_semantics=None):
        seg = input

        x = F.interpolate(seg, size=(self.sh, self.sw))
        x = self.fc(x)
        
        if style_semantics is not None:
            #import pdb; pdb.set_trace()
            style_codes = self.Zencoder(input=rgb_img, segmap=style_semantics)
        else:    
